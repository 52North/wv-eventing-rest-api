...
public class AccessRightsImpl implements AccessRights, InitializingBean {
    ...
    public boolean canSeeSubscriptionsOfUser(WvUser u1, WvUser u2) {
        ...
        if (isInAdminGroup(u1)) {
            return true;
        }

        // DISCUSS: if it is not the same user (and this is not a
        // group subscription), access should be denied
        return false;
//        return u2.getGroups().stream()
//                .filter((Group g) -> isGroupAdmin(u1, g))
//                .findAny().isPresent();
    }
...
//        }
    ...
    public boolean isInAdminGroup(WvUser u) {
        if (u.getGroups() != null) {
            if (u.getGroups().stream().anyMatch((group) -> {
                        return adminGroups.contains(group.getName());
                    })) {
                return true;
            }
        }
    ...
    @Override
    public boolean isGroupAdmin(WvUser u, int groupId) {
        return u.hasAdminGroups()
                && u.getAdminGroups().stream().filter(g -> g.getId() == groupId).findFirst().isPresent();
    }

    @Override
    public boolean canCreateNotifications(WvUser user) {
        return isInAdminGroup(user);
    }

    @Override
    public boolean withinAdminGroupNames(String name) {
        return this.adminGroups.contains(name);
    }

    @Override
    public WvUser enhanceUser(WvUser u) {
        u.setAdmin(isInAdminGroup(u));
        u.getGroups().forEach(g -> g.setGroupAdmin(isGroupAdmin(u, g)));
        return u;
    }
