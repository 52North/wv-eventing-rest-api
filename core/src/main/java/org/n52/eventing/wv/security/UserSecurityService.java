...
public class UserSecurityService implements AuthenticationProvider, Serializable, SecurityService {
    ...
    public User resolveCurrentUser() throws NotAuthenticatedException {
        WvUser result = resolveCurrentWvUser();
        if (result != null) {
            return new UserWrapper(result, containsAdminGroup(result.getGroups()));
        }

        throw new NotAuthenticatedException("No valid user object found");
    }

    public WvUser resolveCurrentWvUser() throws NotAuthenticatedException {
        ...
        if (auth != null) {
            ...
            try {
                ...
                return accessRights.enhanceUser(result.get());
            }
            finally {
                session.close();
            }
